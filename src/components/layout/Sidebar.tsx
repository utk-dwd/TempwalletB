...
interface SidebarProps {
  ...
  const [showPolicyPopup, setShowPolicyPopup] = useState<string | null>(null);  // Use react-router-dom hooks
  const location = useLocation();
  const navigate = useNavigate();  // Map nav items to their corresponding routes
  ...
  };  const activeItem = getActiveItem();  const handleNavClick = (item: string, event?: React.MouseEvent) => {
    analyticsService.trackEvent(EventName.NAVIGATION_CLICKED, { destination: item });
    const comingSoonItems = ['Address Book', 'History', 'Settings'];if (comingSoonItems.includes(item)) {
  if (event) {
  ...
  if (onNavClick) {
    ...
    <>
      <div className="sidebar w-[280px] h-auto p-4 flex flex-col rounded-xl mb-5 mt-5 ml-5 border border-white/20">
        ...
        </Tooltip.Provider>    <nav className="space-y-2 flex-1 flex-grow">
      {navItems.map((item) => (
        <Button
          key={item}
          variant="ghost"
